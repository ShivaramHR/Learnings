...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """                             # Eg
    L1 = list(freq_dict1.keys())    #['hello', 'world', 'hello']
    L2 = list(freq_dict2.keys())    # ['hello', 'friends']
    U = list(set(L1)|set(L2))       # ['friends', 'world', 'hello']
    delta = []
    sigma = []

    # This is a brute force way of solving this .
    ...
    #     elif element not in L1 and element in L2:
    #         delta.append(freq_dict2[element])
    #         sigma.append(freq_dict2[element])
    #     elif element in L1 and element not in L2:
    #         delta.append(freq_dict1[element])
    #         sigma.append(freq_dict1[element])
    
    # This is more clear and readable
    for element in U:
        count1 = freq_dict1.get(element, 0)
        count2 = freq_dict2.get(element, 0)

        delta.append(abs(count1 - count2))
        sigma.append(count1 + count2)
    
    if sum(sigma) == 0:
        return 1.0
    similarity = 1 - (sum(delta)/sum(sigma))
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for word in all_words:
        union_dict[word] = freq_dict1.get(word, 0) + freq_dict2.get(word, 0)
    if not union_dict:
        return []
    highest = max(union_dict.values())
    freq_words = []
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for word, doc_count in word_doc_count.items():
        idf = math.log10(total_docs/doc_count)
        idf_dict[word] = idf
