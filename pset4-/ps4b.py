...
class Message(object):
    ...
    def shift_char(self, char, shift):
        ...
        '''
        ascii_value = 32 + ((ord(char) + shift -32)%95)
    ...
    def apply_pad(self, pad):
        ...
        for i in range(len(self.text)):
            new_char = self.shift_char(self.text[i], pad[i])
            result += new_char
        return result
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        for num in pad:
            new_pad.append(-num)
        decrypted_text = Message.apply_pad(self, new_pad)
        return PlaintextMessage(decrypted_text, pad)
        raise NotImplementedError  # delete this line and replace with your code here
m = Message("test")
print(m.shift_char('A', 3))   # Expected: 'D'
print(m.shift_char('~', 1))   # Expected: ' '
print(m.shift_char(' ', -1))  # Expected: '~'
print(m.shift_char('A', 200))  # Should still return a valid printable character
m1 = Message("ABC")
pad = [1, 2, 3]
print(m1.apply_pad(pad)) # prints BDF
p = PlaintextMessage("HELLO")
print(len(p.get_pad()) == len("HELLO"))  # Expected: True
print(p.get_ciphertext())
p1 = PlaintextMessage("HELLO")
old_cipher = p.get_ciphertext()

new_pad = [1,1,1,1,1]
p1.change_pad(new_pad)

print(p1.get_ciphertext() != old_cipher)  # Expected: True
